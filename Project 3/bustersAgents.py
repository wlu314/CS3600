    ...
    def chooseAction(self, gameState):
        ...
        livingGhostPositionDistributions = \
            [beliefs for i, beliefs in enumerate(self.ghostBeliefs)
             if livingGhosts[i+1]]
        "*** YOUR CODE HERE ***"
        ## 1. calculate the likely position of ghosts, stored in 
        ## 2. find the closest one

        likely_pos = None ## closests ghost 
        closests_dis = float('inf') ## the distance to that ghost 
        for distribution in livingGhostPositionDistributions:
            best = distribution.argMax()
            distance = self.distancer.getDistance(pacmanPosition, best)
            ## set the likely ghost and closest
            if distance <= closests_dis:
                closests_dis = distance
                likely_pos = best

        
        ## 3. then choose action to get to the closests one 
        best_action = None
        min_distance = float('inf')
        for action in legal:
            successor_pos = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successor_pos, likely_pos)

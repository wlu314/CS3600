...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for pos in self.allPositions:
            prob = self.getObservationProb(noisy_distance, pacman_pos, pos, jail_pos)
            ## calculates the cases as well as noisy manhanthan distance 
    ...
    def elapseTime(self, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        discrete_dis = DiscreteDistribution() ##nodels belief distributions and weight distributions
        for pos in self.allPositions:
            ## iterate over all positions to get new position based on beliefs
            dict = self.getPositionDistribution(gameState, pos)
            b = self.beliefs[pos]
            for new_pos in dict:
                discrete_dis[new_pos] += dict[new_pos] * b ## multiply prob of new position to orginal to get new distribution
        self.beliefs = discrete_dis ## set 
    def getBeliefDistribution(self):
        return self.beliefs
...
class ParticleFilter(InferenceModule):
    ...
    def initializeUniformly(self, gameState):
        ...
        "*** YOUR CODE HERE ***"
        legal_pos = self.legalPositions ## legal pos itions to index to 
        legal_pos_len = len(legal_pos)
        
        for i in range(self.numParticles):
            index = i % legal_pos_len
            curr_pos = legal_pos[index]
            self.particles.append(curr_pos) ## evenly by using list
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        ## same as exact inference one
        pacman_pos = gameState.getPacmanPosition() 
        jail_pos = self.getJailPosition()  
        noisy_distance = observation
        particle_len = self.numParticles
        ## weight distribution over self.particles where weight of particle is probability
        weights = DiscreteDistribution()
        for particle in self.particles:
            weights[particle] = self.getObservationProb(noisy_distance, pacman_pos, particle, jail_pos) + weights[particle]
            ## returns probabilty give npacman's position and that particle location 
        ## if the weights total to zero, i need to reinitilize
        if weights.total() == 0: 
            self.initializeUniformly(gameState)
        ## else just resample distribution
        else:
            weights.normalize()
            particle = []
            for i in range(particle_len):
                sampled_weight = weights.sample()
                particle.append(sampled_weight)
    ...
    def elapseTime(self, gameState):
        ...
        for oldPos in self.particles: 
            newPosDist = self.getPositionDistribution(gameState, oldPos) ## given 
            new_particle_list.append(newPosDist.sample()) ## using the infernece module
    ...
    def getBeliefDistribution(self):
        ...
        for particle in self.particles: 
            distribution[particle] = distribution[particle] + 1 ## counts to them be normalized aka get distribution 
        distribution.normalize()
        return distribution ## retruns normalized distribution

...
class Perceptron(object):
    ...
    def sigmoidActivation(self, inActs):
        ...
        """
        """YOUR CODE"""
        inActs = [1] + inActs ## bias inputs
        weight_sum = self.getWeightedSum(inActs)
    ...
    def sigmoidActivationDeriv(self, inActs):
        ...
        inActs = [1] + inActs
        weight_sum = self.getWeightedSum(inActs)
        return self.sigmoidDeriv(weight_sum)
    ...
    def updateWeights(self, inActs, alpha, delta):
        ...
        for i in range(len(self.weights)):
            self.weights[i] = self.weights[i] + alpha * delta * inActs[i]
            totalModification = totalModification + abs((alpha * delta * inActs[i]))
...
class NeuralNet(object):
    ...
    def feedForward(self, inActs):
        ...
        for layer in self.layers:
            next_input = []
            for unit in layer:
                next_input.append(unit.sigmoidActivation(output[-1]))
        ...
        for example in examples:#for each example
            ...
            for layerNum in range(self.numHiddenLayers-1,-1,-1):
                ...
                for neuronNum in range(len(layer)):
                    ...
                    delta = gPrime * sum(nextLayer[nextNeuron].weights[neuronNum+ 1]* deltas[0][nextNeuron] for nextNeuron in range(len(nextLayer)))
...
def buildNeuralNet(examples, alpha=0.1, weightChangeThreshold = 0.00008,hiddenLayerList = [1], maxItr = sys.maxsize, startNNet = None):
    ...
    while maxItr > iteration and weightMod > weightChangeThreshold: ## given condition for stop trian
        average_error, average_weight_change = nnet.backPropLearning(examplesTrain, alpha)
        trainError = average_error
        weightMod = average_weight_change
        iteration+=1
        ## ! on iteration 10; training error 0.006085 and weight change 0.000272
        if iteration % 10 == 0:
            print(f'! on iteration {iteration}; training error {average_error:.6f} and weight change {average_weight_change:.6f}')
        else:
            print('.')
    time = datetime.now().time()
    ...
    for input, output in examplesTest:
        NN_outputs = nnet.feedForward(input)[-1]
        rounded_outputs = []
        for out in NN_outputs:
            rounded_outputs.append(round(out))
        
        if rounded_outputs == output:
            testCorrect += 1
        else:
            testError += 1
